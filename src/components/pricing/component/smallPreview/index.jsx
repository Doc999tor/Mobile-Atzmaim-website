...
export default ({ name, icon, value, switchValue, handleShowOpenPlan }) => {
	const basic = name === 'basic'
	return (
		<div id={`${name}`} class={style.price + ' ' + (basic ? style.active : '')} onClick={() => handleShowOpenPlan(name)}>
			...
			<div class={style.price_info}>
				...
				<p class={style.price_plan}><span class={style.currency}>{config.translations.pricing.data[name].small_preview.group_preview_price.replace('{currency}', config.modules.pricing.currency)
					.replace('{price_value}', value)}</span>{!switchValue && <span class={style.per}>{config.translations.pricing.data[name].small_preview.period}</span>}</p>
			</div>
		</div>
	);
};
