					...
					<div class={styles.background_top}>
						{!animation
							? <Fragment>
								...
								<img class={config.isRTL ? style.outer_rtl : style.outer_ltr} src={config.urls.media + 'man_active.svg'} />
							</Fragment>
							: <Fragment>
								<img class={config.isRTL ? styles.inner_rtl : styles.inner_ltr} src={config.urls.media + 'bg_top.svg'} />
								<img class={config.isRTL ? style.outer_rtl : style.outer_ltr} src={config.urls.media + 'screen.svg'} />
							</Fragment>
						}
						{
							!animation && <img class={config.isRTL ? style.man_rtl : style.man_ltr} src={config.urls.media + 'man.svg'} />
						}
						{!animation
							? <img class={config.isRTL ? style.man_start_rtl : style.man_start_ltr} src={config.urls.media + 'man.svg'} />
							: <img class={config.isRTL ? `${animation && style.man_end_rtl}` : `${animation && style.man_end_ltr}`} src={config.urls.media + 'man_active.svg'} />
						}
						{!animation
							? <img class={config.isRTL ? style.coin_start_rtl : style.coin_start_ltr} src={config.urls.media + 'coin.svg'} />
							: <img class={config.isRTL ? style.coin_rtl : style.coin_ltr} src={config.urls.media + 'coin.svg'} />
						}
					</div>
				</section>
