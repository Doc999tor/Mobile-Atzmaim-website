...
export default class AddFeedback extends Component {
	...
	rating = (rating, icon) => (
		Array.from({ length: rating }).map((r, i) => <img class={style.star} key={i} src={config.urls.media + icon} alt='star' />)
	);
						...
						{!this.state.imgUrl
							? <label class={style.label}>
								<div class={style.upload}>
									<img class={style.picture} src={config.urls.media + 'ic_upload_photo.svg'} alt='upload photo' />
								</div>
							...
							: <div class={style.added_photo}>
								<img class={style.uploaded_photo} height='50' width='50' loading='lazy' src={this.state.imgUrl} alt='uploaded photo' />
								<p class={style.uploaded_text}>{config.translations.feedback.uploaded_photo}<img class={style.mark} src={config.urls.media + 'ic_check_mark.svg'} alt='check mark' /></p>
							</div> }
					</div>
