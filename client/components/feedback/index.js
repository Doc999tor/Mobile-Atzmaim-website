...
export default class Reviews extends Component {
	state = {
		addFeedback: false,
		feedback: this.props.config.modules.feedback.data
	}
	...
	render ({ animation, translations, config }) {
		...
		return (
			<div id='feedback' class='height'>
				{addFeedback
					? <AddFeedback translations={translations} config={config} cancel={this.handleToggleForm} onFormSubmit={this.onFormSubmit} />
					: <div>
						<section class='top_section'>
							<div class={`text ${animation && (config.isRTL ? 'topSection_rtl' :'topSection_ltr')}`}>
								<h2 class={!animation ? 'hidden_content' : ''}>{translations.feedback.main_title}</h2>
								<div class={!animation ? 'hidden_content' : 'types_actions'}>
									...
									<p>{translations.feedback.leave_btn_label}</p>
								</div>
							</div>
						...
						<section class={`types_wrap ${animation && (config.isRTL ? 'topSection_rtl' :'topSection_ltr')}`}>
							<div class={!animation ? 'hidden_content' : ''}>
								<h3 class='subtitle'>{translations.feedback.subtitle}</h3>
								<div class='reviews_wrap'>
									{feedback.map(item => <Feedback config={config} icon={item.picture_web} name={item.customer_name} rating={item.rating} text={item.text} />)}
								</div>
							</div>
						</section>
					</div>}
